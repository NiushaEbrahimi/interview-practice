"""
DEPRECATED: Use seed_from_json instead.

This command is kept for backwards compatibility.
New questions should be added to JSON files in api/seed_data/.

Usage:
    python manage.py seed_from_json                    # seed from all JSON files
...
class Command(BaseCommand):
    help = (
        "DEPRECATED: Use 'seed_from_json' instead. "
        "This command seeds minimal fallback questions for existing lessons."
    )

    def handle(self, *args, **kwargs):
        self.stdout.write(self.style.WARNING(
            "This command is deprecated. Use 'python manage.py seed_from_json' instead.\n"
            "See api/seed_data/ for JSON seed files.\n"
        ))

        # Minimal fallback seed for backwards compatibility
        questions_map = {
            "React Introduction": [
        ...
        for lesson in lessons:
            ...
            for q, a in q_list:
                if not Question.objects.filter(lesson=lesson, question=q).exists():
        ...
        self.stdout.write(self.style.SUCCESS(
            f"Created {created} questions (legacy fallback). "
            f"Use 'seed_from_json' for full question sets."
        ))
